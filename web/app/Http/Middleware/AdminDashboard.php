...
class AdminDashboard
{
    ...
    public function handle($request, Closure $next)
    {
        ...
        if (!auth()->guest() && auth()->user()->isAbleTo('admin')) {
            ...
            if ($request->is('admin/*') || $request->route()->getPrefix() == "/admin" ||
                ...
                if (empty($config)) {
                    ...
                    $config->skin = '{ "skin" : "skin-blue", "data": ["dark","navbar-lightblue","bg-lightblue","navbar-lightblue","sidebar-dark-lightblue"]}';
                    $config->sidebar = true;
                    $config->save();
                }
