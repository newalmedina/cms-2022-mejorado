...
{
    public const APITOKEN = "API_ACCESS_TOKEN";
     ...
     *                     type="string"
     *                 ),
     *                 example={"email": "ing.newal.medina@outlook.com"}
     *             )
     *         )
     *     ),
     *     @OA\Response(
    ...
    public function postEmail(Request $request): JsonResponse
    {
        ...
        try {
            ...
            $token_email = app('auth.password.broker')->createToken($user);
            // $app = Password::broker();
            ...
            // Enviar mail
            Mail::send(
                'modules.email.reset_password',
                compact('codigo_envio'),
                function ($message) use ($user) {
                    $message
                        ->to($user->email)
                        ->subject('Reset password code' . env("APP_NAME"));
                }
            );
