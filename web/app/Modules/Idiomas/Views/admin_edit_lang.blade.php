        ...
        <div class="tab-content" id="custom-tabs-langContent">
            ...
            @foreach ($a_trans as $key => $valor)
                    <div class="tab-pane fade show @if ($nX == 1) active @endif"
                        ...
                        {!! Form::hidden('lang[' . $key . '][id]', $valor['id'], ['id' => 'id']) !!}

                        {{-- Text Lang - name --}}
                        <div class="form-group row">
                            {!! Form::label('lang[' . $key . '][name]', trans('Idiomas::idiomas/admin_lang.fields.name'), ['class' => 'col-sm-2 control-label']) !!}
                            <div class="col-sm-10">
                                {!! Form::text('lang[' . $key . '][name]', $idioma->{'name:' . $key}, ['placeholder' => trans('Idiomas::idiomas/admin_lang.fields.name_helper'), 'class' => 'form-control', 'id' => 'name_' . $key]) !!}
                            </div>
                        </div>
                    </div>
                    <?php
