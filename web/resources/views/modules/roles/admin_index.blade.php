        ...
        <div class="col-12">
            <div class="card card-secondary card-outline">
                <div class="card-header with-border">
                    <h3 class="card-title">{{ trans("roles/lang.listado_roles") }}</h3>
                </div>

                <div class="card-body">
                    @if(Auth::user()->isAbleTo("admin-roles-create"))
                        <a href="{{ url('admin/roles/create') }}" class="btn btn-success float-right"><i class="fa fa-plus-circle" aria-hidden="true"></i> {{ trans('roles/lang.nuevo_roles') }}</a>
                    @endif
                </div>

                <!-- /.card-header -->
                <div class="card-body">
                    <table id="table_roles" class="table table-bordered table-striped" aria-hidden="true">
                        <thead>
                        <tr>
                            <th scope="col"></th>
                            <th scope="col"></th>
        ...
        $(function () {
            oTable = $('#table_roles').DataTable({
                ...
                columns: [
                    ...
                    {
                        "title"         : "{!! trans('roles/lang.nombre_roles') !!}",
                        orderable       : true,
                        searchable      : true,
                    ...
                    {
                        "title"         : "{!! trans('roles/lang.description') !!}",
                        orderable       : true,
                        searchable      : true,
                    ...
                    {
                        "title"         : "{!! trans('roles/lang.acciones') !!}",
                        orderable       : false,
                        searchable      : false,
                        width          : '110px',
                        data            : 'actions'
            ...
            $('tfoot th',$('#table_roles')).each( function (colIdx) {
                var title = $('tfoot th',$('#table_roles')).eq( $(this).index() ).text();
            ...
            $('#table_roles').on( 'keyup change','tfoot input', function (e) {
                oTable
            ...
            $.ajax({
                ...
                success : function(data) {
                    ...
                    if(data) {
                        ...
                        $("#alertModalHeader").html("Borrado de roles");
