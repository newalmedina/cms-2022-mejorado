...
class CreateCrudGeneratorTables extends Migration
{
    ...
    public function up()
    {
        Schema::create('crud_themes', function (Blueprint $table) {
            $table->increments('id');
            $table->string('name');
            $table->string('slug');
        ...
        Schema::create('crud_types', function (Blueprint $table) {
            $table->increments('id');
            $table->string('name');
            $table->string('slug');
            $table->boolean('active')->default(true);
        ...
        Schema::create('crud_modules', function (Blueprint $table) {
            ...
            $table->foreign('theme_id')->references('id')->on('crud_themes')->onDelete('cascade');
            $table->foreign('type_id')->references('id')->on('crud_types')->onDelete('cascade');
        });
        ...
        Schema::create('crud_module_fields', function (Blueprint $table) {
            $table->increments('id');
            $table->integer('order_list')->unsigned()->default(0);
            $table->integer('order_create')->unsigned()->default(0);
        ...
        Schema::create('crud_module_grid', function (Blueprint $table) {
            $table->id();
            $table->integer('crud_module_id')->unsigned();
            $table->integer('crud_module_field_id')->unsigned()->nullable();
            $table->integer('row')->unsigned();
            $table->integer('col')->unsigned();
            $table->integer('max_col')->unsigned()->default(1);

            $table->timestamps();


            $table->foreign('crud_module_id')->references('id')->on('crud_modules')->onDelete('cascade');
            $table->foreign('crud_module_field_id')->references('id')->on('crud_module_fields')->onDelete('cascade');
        });
    }
