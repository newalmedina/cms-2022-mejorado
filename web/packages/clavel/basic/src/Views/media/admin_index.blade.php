        ...
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    <button data-dismiss="modal" class="btn btn-secondary" type="button">{{ trans('general/admin_lang.close') }}</button>
                    <button onclick="javascript:$('#formFolder').submit();" class="btn btn-primary" type="button">{{ trans('general/admin_lang.save') }}</button>
                </div>
            </div><!-- /.modal-content -->
        </div><!-- /.modal-dialog -->
    </div>
        ...
        <div class="col-md-3">
            ...
            <div class="card card-primary card-outline">
                <div class="card-header">
                    <h3 class="card-title">{{ trans("basic::media/admin_lang.folders") }}</h3>
                </div>
                <div class="card-body p-0" style="max-height: 75vh; overflow-y: auto;">
                    <ul class="nav nav-pills nav-pills-flat flex-column">
                        <li id="li_root" class="nav-item list_folder active">
                            <a class="nav-link" href="javascript:changeFolder('/', 'root');">
                                <i id="fa_root" class="fas fa-folder-open text-secondary fa-icon_f pr-2"></i>/
                            </a>
                        </li>
                        @foreach($subfolders as $key => $value)
                            <li id="li_{{ $key }}" class="nav-item list_folder">
                                <a class="nav-link" href="javascript:changeFolder('{{ $value }}', '{{$key}}');">
                                    {!! str_repeat('&nbsp;',(substr_count($value,'/')) * 5) !!}
                                    <i id="fa_{{ $key }}" class="fas fa-folder text-secondary fa-icon_f pr-2" aria-hidden="true"></i>
                                    {{ basename($value) }}
                                    <button onclick="deletedir('{{ $value }}');" class="btn btn-xs btn-danger float-right"><i class="fa fa-trash" aria-hidden="true"></i></button>
                                </a>
                            </li>
                        @endforeach
                    </ul>
                </div>
                @if(Auth::user()->isAbleTo("admin-media-create"))
...
@section("foot_page")
    ...
    <script type="text/javascript">
        ...
        $(document).ready(function() {
            ...
            oTable = $('#table_media').DataTable({
                ...
                columns: [
                    ...
                    {
                        "title"         : "{!! trans('basic::media/admin_lang.tamaño') !!}",
                        orderable       : false,
                        searchable      : false,
                        data: 'size', name            : 'size',
                        sWidth          : '100px'
        ...
        function optimizeVideo(url) {
            ...
            strBtn+= '<button type="button" id="optimizeVideoCancel" class="btn btn-secondary" data-dismiss="modal">{{ trans('general/admin_lang.close') }}</button>';
                ...
                success : function(data) {
                    ...
                    if(data) {
                        if(data.success) {
                            ...
                            $("#alertModalBody").html("<i class='fas fa-times-circle text-danger' style='font-size: 64px; float: left; margin-right:15px;'></i> " + data.msg);
                            $("#modal_alert").modal('toggle');
                        }
                    } else {
                        $("#modal_alert").addClass('modal-danger');
                        $("#alertModalBody").html("<i class='fas fa-bug text-danger' style='font-size: 64px; float: left; margin-right:15px;'></i> {{ trans('basic::media/admin_lang.errorajax') }}");
                        $("#modal_alert").modal('toggle');
                    }
                    return false;
                }
