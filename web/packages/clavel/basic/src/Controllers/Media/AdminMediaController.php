...
class AdminMediaController extends AdminController
{
    ...
    public function index()
    {
        ...
        if (!auth()->user()->isAbleTo('admin-media-list')) {
            app()->abort(403);
        }
    ...
    public function getData($path = 'root')
    {
        ...
        return Datatables::of($media)
            ...
            ->addColumn('actions', function ($row) {
                ...
                if (auth()->user()->isAbleTo("admin-media-delete")) {
                ...
                if (auth()->user()->isAbleTo("admin-media-update") && $row->mime == "video/mp4") {
    ...
    public function destroy($id)
    {
        ...
        if (!auth()->user()->isAbleTo('admin-media-delete') && $media->user_id==auth()->user()->id) {
            app()->abort(404);
        }
    ...
    public function optimize($id)
    {
        ...
        // Si no tiene permisos para borrar lo echamos
        if (!auth()->user()->isAbleTo('admin-media-update') && $media->user_id==auth()->user()->id) {
            app()->abort(404);
        }
