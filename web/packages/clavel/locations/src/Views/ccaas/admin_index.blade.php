        ...
        <div class="col-12">
            <div class="card card-secondary card-outline">
                <div class="card-header with-border">
                    <h3 class="card-title">{{ trans('locations::ccaas/admin_lang.listado') }}</h3>
                </div>
                ...
                <div class="card-body">
                    ...
                    <div class="float-right">
                        @if(Auth::user()->isAbleTo("admin-ccaas-create"))
                            <a href="{{ url('admin/ccaas/create') }}" class="btn btn-success float-right"><i class="fa fa-plus-circle" aria-hidden="true"></i> {{ trans('locations::ccaas/admin_lang.nueva') }}</a>
                        @endif
                    </div>
                </div>
                ...
                <!-- /.card-header -->
                <div class="card-body">
                    <table id="table_ccaas" class="table table-bordered table-striped" aria-hidden="true">
                        <thead>
                        <tr>
                            <th scope="col">
                            <th scope="col">
...
<th scope="col">
    ...
    <div class="col-12">
        <div class="card">
            <div class="card-header"><h3 class="card-title">{{ trans("locations::ccaas/admin_lang.export") }}</h3></div>
            <div class="card-body">
                <a href="{{ url('admin/ccaas/export') }}" class="btn btn-app">
                    <i class="far fa-file-excel" aria-hidden="true"></i>
                    {{ trans('locations::ccaas/admin_lang.exportar_datos') }}
                </a>
            </div>
        </div>
    </div>
...
@section("foot_page")
        ...
        $(function () {
            oTable = $('#table_ccaas').DataTable({
                ...
                columns: [
                    ...
                    {
                        "title"         : "{!! trans('locations::ccaas/admin_lang.fields.active') !!}",
                        orderable       : false,
                        searchable      : false,
                        data            : 'active',
                        sWidth          : '50px'
                        ...
                        {
                                "title"         : "{!! trans('locations::ccaas/admin_lang.fields.name') !!}",
                                orderable       : true,
                                searchable      : true,
                                data            : 'name',
                                name            : 'ct.name',
                        ...
                        ,
                    {
                        "title"         : "{!! trans('locations::ccaas/admin_lang.acciones') !!}",
                        orderable       : false,
                        searchable      : false,
                        sWidth          : '110px',
                        data            : 'actions'
            ...
            $('tfoot th',$('#table_ccaas')).each( function (colIdx) {
                var title = $('tfoot th',$('#table_ccaas')).eq( $(this).index() ).text();
            ...
            $('#table_ccaas').on( 'keyup change','tfoot input', function (e) {
                oTable
    ...
    $.ajax({
        ...
        $('#table_ccaas tbody').on('change', 'input[type="checkbox"]', function(){
        ...
        function deleteSelected() {
            ...
            if(ids.length>0){
                deleteElements('{{ url('admin/ccaas/delete-selected') }}', ids);
            }
        }

        function getSelectedRecords() {
            var ids = [];
            $('#table_ccaas input[type="checkbox"]').each(function(){
