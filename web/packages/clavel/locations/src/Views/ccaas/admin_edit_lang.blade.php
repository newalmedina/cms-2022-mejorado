            ...
            @foreach ($a_trans as $key => $valor)
                <li class="nav-item">
                    <a class="nav-link  @if ($nX == 1) active @endif" id="tab_{{ $key }}_tab" data-toggle="pill"
                        ...
                        aria-selected="true">
                        {{ $valor['idioma'] }}
                        @if($nX==1)- <span class="text-success">{{ trans('locations::ccaas/admin_lang.defecto') }}</span>@endif
                    </a>
                </li>
                <?php
                $nX++;
...
<div class="form-group row">
    {!! Form::label('lang['.$key.'][name]', trans('locations::ccaas/admin_lang.fields.name'), array('class' => 'col-sm-2 col-form-label')) !!}
    <div class="col-sm-10">
        {!! Form::text('lang['.$key.'][name]', $ccaa->{'name:'.$key} , array('placeholder' => trans('locations::ccaas/admin_lang.fields.name_helper'), 'class' => 'form-control', 'id' => 'name_'.$key)) !!}
    </div>
</div>
